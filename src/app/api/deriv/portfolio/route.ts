...
function wsCall(token: string, payload: object, matcher: (m: any) => boolean, ms = 18000): Promise<any> {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await wsCall(token, { portfolio: 1 }, (m) => m.msg_type === 'portfolio');
    return NextResponse.json({ success: true, contracts: data.portfolio?.contracts ?? [] });
  } catch (e: any) {
    return NextResponse.json({ success: false, error: e.message, contracts: [] });
  }
}