...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await wsCall(token, { balance: 1, account: 'all' }, (m) => m.msg_type === 'balance');
    return NextResponse.json({ success: true, balance: data.balance?.balance ?? 0, currency: data.balance?.currency ?? 'USD' });
  } catch (e: any) {
